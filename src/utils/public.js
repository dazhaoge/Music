...
export default {
  ...
  */
   ...
   * @return {Number} [返回不等于当前值的随机整数]
  */
  randomNumber(min, max, current){
    var random = parseInt(Math.random()*(max-min+1)) + min;
    if(random == current) {
      this.randomNumber(min, max, current);
    }
    return random
  },
  ...
  */
  getParams() {
    let locationHref = location.href; //取得整个地址栏
    let paramsIndex = locationHref.indexOf("?"); //截取参数位置
    let hrefParams = locationHref.slice(paramsIndex + 1); //截取所有参数
    let paramsArr = hrefParams.split("&");
    let paramsObj = {}
